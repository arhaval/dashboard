...
function unmeasuredCheckpoints(): PublicationCheckpoint[] {
  return CHECKPOINTS.map((key) => ({
    ...
    measurementQuality: 'PARTIAL_SOURCE_DATA' as const,
    sourceGranularity: 'REALTIME' as const,
      ...
      for (const d of drafts) {
        ...
        d.checkpoints = publicationMetricsService
          ...
          .map((c) => ({
            ...
            isLate: c.isLate,
            status: c.status,
            measurementQuality: c.measurementQuality,
            sourceGranularity: c.sourceGranularity,
            dataThroughDate: c.dataThroughDate,
            isBackfilled: c.isBackfilled,
            isSourceDataComplete: c.isSourceDataComplete,
