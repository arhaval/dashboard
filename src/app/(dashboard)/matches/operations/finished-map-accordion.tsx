    ...
    <div className="border border-[#2A2A2A] rounded-[4px] overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2">
          ...
          <span className="text-[#A1A1A1]">Map {map.map_number}:</span>
          <span className="text-[#FAFAFA]">{map.map.replace('de_', '')}</span>
        </span>
        <span className="flex items-center gap-2">
          <span className="text-[#FAFAFA] font-mono">{map.team1_score} - {map.team2_score}</span>
      ...
      <table className="w-full text-xs font-mono">
        ...
        <tbody>
          {sorted.map((p, i) => {
            ...
            return (
              <tr key={i} className={i % 2 === 0 ? 'bg-[#141414]' : 'bg-[#0F0F0F]'}>
                <td className="py-1 text-[#FAFAFA] truncate max-w-[120px]">{p.player_name}</td>
                <td className="text-right py-1 text-[#FAFAFA]">{p.kills}</td>
                <td className="text-right py-1 text-[#A1A1A1]">{p.deaths}</td>
                <td className="text-right py-1 text-[#A1A1A1]">{p.assists}</td>
                <td className="text-right py-1 text-[#FAFAFA]">{adr.toFixed(1)}</td>
                <td className="text-right py-1 text-[#FF4D00]">{kda.toFixed(2)}</td>
              </tr>
            );
          })}
        </tbody>
