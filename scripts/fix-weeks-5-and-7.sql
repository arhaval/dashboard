-- Week 5 ve Week 7 Düzeltme
-- Week 5 (2026-04-14): 2 forfeit + 3 gerçek maç = 5 maç
-- Week 7 (2026-04-28): 4 pending maç (BHEAM vs METAL hariç orijinal fikstür)

BEGIN;

-- 1) Week 5'teki (2026-04-14) tüm ilgili kayıtları temizle (forfeit'ler hariç)
DO $$
DECLARE
  t_gegen UUID; t_busc UUID;
  t_mamba UUID; t_sik UUID;
  t_crimson UUID; t_boru UUID;
  t_ak47 UUID; t_hizal UUID;
BEGIN
  ...
  -- Week 5'teki ilgili 3 maçı (ve player stats/map_maps) temizle
  DELETE FROM cs2_match_players
  WHERE map_id IN (
    SELECT mm.id FROM cs2_match_maps mm
    JOIN cs2_matches m ON m.id = mm.match_id
    WHERE m.match_date::date = '2026-04-14'
    AND (
      (m.team1_id = t_gegen AND m.team2_id = t_busc) OR (m.team1_id = t_busc AND m.team2_id = t_gegen)
      OR (m.team1_id = t_mamba AND m.team2_id = t_sik) OR (m.team1_id = t_sik AND m.team2_id = t_mamba)
      OR (m.team1_id = t_crimson AND m.team2_id = t_boru) OR (m.team1_id = t_boru AND m.team2_id = t_crimson)
    )
  );
  ...
  -- Week 7'deki (2026-04-28) tüm kayıtları temizle
  DELETE FROM cs2_match_players
  WHERE map_id IN (
    SELECT mm.id FROM cs2_match_maps mm
    JOIN cs2_matches m ON m.id = mm.match_id
...
-- 2) Week 5 için 3 gerçek maçı ekle
DO $$
DECLARE
  t_gegen UUID; t_busc UUID;
  t_mamba UUID; t_sik UUID;
...
BEGIN
  ...
  -- ==========================================
  -- WEEK 7 (2026-04-28) - 4 PENDING MAÇ (BHEAM vs METAL hariç)
  -- ==========================================
  INSERT INTO cs2_matches (team1_id, team2_id, status, match_date)
  VALUES (t_crimson, t_busc, 'PENDING', '2026-04-28');

  INSERT INTO cs2_matches (team1_id, team2_id, status, match_date)
  VALUES (t_ak47, t_sik, 'PENDING', '2026-04-28');

  INSERT INTO cs2_matches (team1_id, team2_id, status, match_date)
  VALUES (t_hizal, t_boru, 'PENDING', '2026-04-28');

  INSERT INTO cs2_matches (team1_id, team2_id, status, match_date)
  VALUES (t_mamba, t_gegen, 'PENDING', '2026-04-28');

  RAISE NOTICE 'Düzeltme tamamlandı: Week 5 (5 maç), Week 7 (4 pending maç)';
END $$;
